...
基础地址：`http://127.0.0.1:8000`。除登录、注册和 `/api/blog/` 公共接口外，请求头需包含：
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/blog/dynamics/` | 已发布内容分页 |
| GET | `/api/blog/dynamics/{id}/` | 已发布内容详情，不修改阅读量 |
| GET | `/api/blog/dynamics/hot/?limit=6` | 热门内容，`limit` 为 1–100 |
| GET | `/api/blog/dynamics/recent/?limit=6` | 最近内容 |
| PUT | `/api/blog/dynamics/{id}/view/` | 增加阅读量 |
| POST | `/api/blog/dynamics/{id}/like/` | 按访问 IP 点赞 |
| GET/POST | `/api/blog/comments/` | `dynamic_id` 查询已通过评论或提交评论 |
| GET | `/api/blog/categories/` | 分类及已发布内容数 |
| GET | `/api/blog/categories/{id}/dynamics/` | 分类内容 |
| GET | `/api/blog/tags/` | 标签列表 |
| GET | `/api/blog/tags/{id}/dynamics/` | 标签内容 |
| GET | `/api/blog/search/?keyword=vue` | 搜索 |
